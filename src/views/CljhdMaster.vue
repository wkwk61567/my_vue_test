            ...
            <v-col :cols="field.cols">
              ...
              <template v-else>
                <component
                  ...
                  :label="field.name"
                  v-bind="field.componentType === 'select'
...
const checkButtonFlags = async () => {
  // 禁用沒有相關權限的按鈕
  isAddDisabled.value = !(await utils.checkFlag("addflag", "cljhd"));
  ...
  isToggleAuditDisabled.value = !(
    (await utils.checkFlag("auditflag", "cljhd")) ||
    (await utils.checkFlag("unauditflag", "cljhd"))
  );
  isExportExcelDisabled.value = !(await utils.checkFlag(
    "excelflag",
    "cljhd"
  ));
};
